...
// 필터 타입 정의
export interface IIncomeWaitingFilter {
  searchTerm?: string;
...
interface IIncomeWaitingState {
  ...
  // 검색 필터
  filter: IIncomeWaitingFilter;
  tempFilter: IIncomeWaitingFilter; // Popover에서 임시로 사용할 필터
  ...
  setFilter: (filter: Partial<IIncomeWaitingFilter>) => void;
  setTempFilter: (filter: Partial<IIncomeWaitingFilter>) => void;
...
// 초기 필터 상태
const initialFilter: IIncomeWaitingFilter = {
...
// 정산 대기 화물 관리 스토어 생성
export const useIncomeWaitingStore = create<IIncomeWaitingState>()(
  persist(
    (set, get) => ({
      // 초기 상태
      waitingOrders: [],
      ...
      // 필터링 액션
      setFilter: (filter: Partial<IIncomeWaitingFilter>) => {
        set((state) => {
      ...
      setTempFilter: (filter: Partial<IIncomeWaitingFilter>) => set((state) => ({
        tempFilter: { ...state.tempFilter, ...filter },
      })),
      ...
      resetFilter: () => {
        const basicFilter: IIncomeWaitingFilter = { ...initialFilter, status: "운송마감" };
        set((state) => {
...
// 필터 적용 함수
const applyFilter = (orders: IBrokerOrder[], filter: IIncomeWaitingFilter): IBrokerOrder[] => {
