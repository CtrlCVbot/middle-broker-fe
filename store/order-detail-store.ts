...
import { OrderStatusType, IOrderDetail } from '@/utils/mockdata/mock-orders-detail';
...
export const useOrderDetailStore = create<IOrderDetailState>((set, get) => ({
  ...
  isActionAvailable: (action: 'edit' | 'cancel' | 'receipt') => {
    ...
    switch (action) {
      ...
      case 'receipt':
        // 하차완료 또는 정산완료 상태에서만 인수증 발급 가능
        return currentStatus === '하차완료' || currentStatus === '정산완료';
