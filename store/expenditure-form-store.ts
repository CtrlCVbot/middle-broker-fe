...
// 정산 폼 데이터 인터페이스
interface IExpenditureFormData {
...
// 정산 폼 스토어 상태 인터페이스
interface IExpenditureFormState {
  isOpen: boolean;                  // 폼 열림 여부
  selectedOrders: IBrokerOrder[];   // 선택된 화물 목록
  formData: IExpenditureFormData;        // 폼 데이터
  ...
  closeForm: () => void;                       // 폼 닫기
  setFormField: <K extends keyof IExpenditureFormData>(
    key: K, 
    value: IExpenditureFormData[K]
  ) => void;  // 폼 필드 설정
  ...
  submitForm: (data: IExpenditureCreateRequest) => void;  // 폼 제출
}

// 초기 폼 데이터
const initialFormData: IExpenditureFormData = {
  shipperName: '',
  businessNumber: '',
  billingCompany: '',
...
// 정산 폼 스토어 생성
export const useExpenditureFormStore = create<IExpenditureFormState>((set, get) => ({
  isOpen: false,
  selectedOrders: [],
