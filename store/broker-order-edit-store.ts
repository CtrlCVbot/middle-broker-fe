...
// 중개 화물 수정 스토어 인터페이스
interface IBrokerOrderEditState {
  // 상태
  orderId: string | null;         // 수정 중인 중개 화물 ID
  originalData: IBrokerOrderDetail | null; // 원본 중개 화물 데이터
  registerData: IBrokerOrderRegisterData;  // 수정 중인 데이터 (registerForm 호환)
  ...
  setOriginalData: (data: IBrokerOrderDetail | null) => void;
  setRegisterData: (data: Partial<IBrokerOrderRegisterData>) => void;
  ...
  convertDetailToRegisterData: (detail: IBrokerOrderDetail) => IBrokerOrderRegisterData;
}

// 기본 등록 데이터
const initialRegisterData: IBrokerOrderRegisterData = {
  vehicleType: '카고',
  weightType: '1톤',
  cargoType: '',
  remark: '',
...
// 중개 화물 수정 스토어 생성
export const useBrokerOrderEditStore = create<IBrokerOrderEditState>((set, get) => ({
  // 초기 상태
  orderId: null,
  originalData: null,
  ...
  setOriginalData: (data: IBrokerOrderDetail | null) => {
    if (data) {
  ...
  setRegisterData: (data: Partial<IBrokerOrderRegisterData>) => {
  ...
  // 중개 화물 상세 데이터를 등록 폼 데이터 형식으로 변환
  convertDetailToRegisterData: (detail: IBrokerOrderDetail): IBrokerOrderRegisterData => {
    // 중량 타입 호환성 확인
