...
import { createSale, checkSaleExists, createPurchase, checkPurchaseExists, createSettlement } from '@/services/broker-settlement-service';
...
export const useBrokerSettlementStore = create<IBrokerSettlementState & {
  ...
  createSettlement: (orderId: string, dispatchId: string) => Promise<IOrderSale | null>;
  ...
  createSettlement: async (orderId: string, dispatchId: string) => {
    try {
      set({ isLoading: true, error: null });
      ...
      const exists = await get().checkOrderClosed(orderId);
      if (exists) {
        set({ isLoading: false, error: '이미 정산이 생성된 주문입니다.', isSaleClosed: true });
        return null;
      }

      // 매입 정산 데이터 생성
      const saleData = await createSettlement(orderId, dispatchId);
      set({ 
        isLoading: false, 
        currentSale: saleData,
        isSaleClosed: true 
      ...
      const errorMessage = error instanceof Error ? error.message : '정산 생성 중 오류가 발생했습니다.';
      set({ isLoading: false, error: errorMessage });
      return null;
    }
  },
