...
export async function GET(
  ...
  try {
    ...
    const adjustments = await db
      .select()
      .from(purchaseBundleAdjustments)
      .where(eq(purchaseBundleAdjustments.bundleId, id))
      .orderBy(purchaseBundleAdjustments.createdAt);
...
export async function POST(
  ...
  try {
    ...
    const [newAdjustment] = await db
      .insert(purchaseBundleAdjustments)
      .values({
        bundleId: id,
...
export async function PUT(
  ...
  try {
    ...
    const existingAdjustment = await db
      ...
      .from(purchaseBundleAdjustments)
      .where(and(
        eq(purchaseBundleAdjustments.id, adjustmentId),
        eq(purchaseBundleAdjustments.bundleId, id)
      ))
      .limit(1);
    ...
    const [updatedAdjustment] = await db
      .update(purchaseBundleAdjustments)
      .set(updateValues)
      .where(eq(purchaseBundleAdjustments.id, adjustmentId))
      .returning();
