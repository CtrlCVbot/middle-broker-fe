...
import { db } from '@/db';
import { purchaseItemAdjustments, purchaseBundleItems } from '@/db/schema/purchaseBundles';
import { 
...
export async function GET(
  ...
  try {
    ...
    const bundleItem = await db
      ...
      .where(eq(purchaseBundleItems.id, id))
      .limit(1);
    ...
    const adjustments = await db
      ...
      .where(eq(purchaseItemAdjustments.bundleItemId, id))
      .orderBy(purchaseItemAdjustments.createdAt);
...
export async function POST(
  ...
  try {
    ...
    const [newAdjustment] = await db
      .insert(purchaseItemAdjustments)
      .values({
        bundleItemId: id,
...
export async function PUT(
  ...
  try {
    ...
    const existingAdjustment = await db
      ...
      .from(purchaseItemAdjustments)
      .where(and(
        eq(purchaseItemAdjustments.id, adjustmentId),
        eq(purchaseItemAdjustments.bundleItemId, id)
      ))
      .limit(1);
    ...
    const [updatedAdjustment] = await db
      .update(purchaseItemAdjustments)
      .set(updateValues)
      .where(eq(purchaseItemAdjustments.id, adjustmentId))
      .returning();
...
export async function DELETE(
  ...
  try {
    ...
    // 개별 화물 추가금 삭제
    await db
      .delete(purchaseItemAdjustments)
      .where(eq(purchaseItemAdjustments.id, adjustmentId));
