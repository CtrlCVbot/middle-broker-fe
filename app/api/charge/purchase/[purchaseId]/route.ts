...
export async function GET(
  ...
  try {
    ...
    // 매출 인보이스 조회
    const sale = await db.query.orderPurchases.findFirst({
      where: eq(orderPurchases.id, saleId),
      with: {
        order: true,
        company: true
      }
...
export async function PATCH(
  ...
  try {
    ...
    // 매출 인보이스 존재 여부 확인
    const existingSale = await db.query.orderPurchases.findFirst({
      where: eq(orderPurchases.id, saleId)
    });
