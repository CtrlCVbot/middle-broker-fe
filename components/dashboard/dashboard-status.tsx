...
const getGroupStyle = (label: GroupLabel) => {
  switch (label) {
    case "운송요청":
      return { color: "bg-sky-500", textColor: "text-sky-800", iconColor: "text-sky-500" };
    case "배차중":
...
export function DashboardStatus() {
  const { statusStats, loading, fetchStatusStats } = useDashboardStore();
        ...
        {loading.statusStats ? (
          ...
          <>
            ...
            <div className="mt-6 space-y-4 border-t pt-4">
              {statusStats.map((stat) => {
                const { iconColor, color } = getGroupStyle(stat.label);
                ...
                return (
                  <div key={stat.label} className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <span className={cn("font-medium", iconColor)}>{stat.label}</span>
                    </div>
                    <div className="flex items-center">
