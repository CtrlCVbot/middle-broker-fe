...
export function BrokerOrderSettlementInfoEditForm({ 
  ...
  const [additionalFeeSectionOpen, setAdditionalFeeSectionOpen] = useState(true); // 추가금 섹션은 기본적으로 열려있게 설정
              ...
              <div className="p-4 space-y-6 border-t">
                ...
                <div className="space-y-3 mt-6 border-t pt-6">
                  <div 
                    ...
                    onClick={() => setAdditionalFeeSectionOpen(!additionalFeeSectionOpen)}
                  >
                    <div className="flex items-center gap-2">
                      <Plus className="h-4 w-4 text-primary" />
                      <h4 className="font-medium">추가금 관리</h4>
                    </div>
                    <ChevronDown className={`h-4 w-4 transition-transform ${additionalFeeSectionOpen ? 'transform rotate-180' : ''}`} />
                  </div>
                  
                  {additionalFeeSectionOpen && (
                    <div className="mt-4">
                      {/* 추가금 타입 버튼 */}
