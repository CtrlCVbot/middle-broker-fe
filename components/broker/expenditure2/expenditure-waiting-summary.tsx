...
interface IExpenditureWaitingSummaryProps {
  selectedOrders: IBrokerOrder[];
  onCreateExpenditure: () => void;
}

const ExpenditureWaitingSummary: React.FC<IExpenditureWaitingSummaryProps> = ({
  selectedOrders,
  onCreateExpenditure,
}) => {
  // 선택된 화물이 없으면 표시하지 않음
  ...
  const shipperGroups = useMemo(() => {
    ...
    selectedOrders.forEach((order: IBrokerOrder) => {
    ...
    <div className="bg-background sticky bottom-0 border-t shadow-md z-10 mt-4">
      <div className="container py-2.5">
        ...
        <div className="flex items-center justify-between mb-2.5">
          ...
          <Button onClick={onCreateExpenditure} size="sm" className="gap-1 h-8">
...
export default ExpenditureWaitingSummary; 