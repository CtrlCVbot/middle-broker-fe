...
// 매입 정산 상태 배열 - 상태 순서대로 정의
export const PURCHASE_STATUS: PurchaseStatusType[] = [
  'WAITING', 
  'MATCHING', 
  'COMPLETED'
];
...
export interface IPurchase {
  ...
  status: PurchaseStatusType;    // 상태 (정산대기, 정산대사, 정산완료)
  ...
  // 지급 차량 정보
  driverId?: string;          // 지급 차량 ID
  driverName: string;         // 지급 차량명
  driverBusinessNumber: string;      // 지급 차량 사업자번호
  driverBankCode?: string;     // 지급 차량 은행코드
  driverBankAccountHolder?: string;     // 지급 차량 계좌주
  driverBankAccountNumber?: string;     // 지급 차량 계좌번호
}
...
export interface IIncomeFilter {
  ...
  status?: PurchaseStatusType;   // 정산 상태
...
// 정산 상태 진행도를 계산하는 함수
export const getPurchaseProgressPercentage = (currentStatus: PurchaseStatusType): number => {
  const currentIndex = PURCHASE_STATUS.indexOf(currentStatus);
  if (currentIndex === -1) return 0;
  
  return (currentIndex / (PURCHASE_STATUS.length - 1)) * 100;
};

// 정산 상태가 특정 상태 이상인지 확인하는 함수
export const isPurchaseStatusAtLeast = (currentStatus: PurchaseStatusType, targetStatus: PurchaseStatusType): boolean => {
  const currentIndex = PURCHASE_STATUS.indexOf(currentStatus);
  const targetIndex = PURCHASE_STATUS.indexOf(targetStatus);
